...
The first thing to do is to find a parametrisation for BaseP1.
        Contrary to the article, the corrected curve already has a
        nonsingular point over ShaskaField.  The coordinates below were
        recovered independently from its normalization conic.
    */
...
K := ShaskaField;
zz := K.1;
assert MinimalPolynomial(zz) eq Parent(MinimalPolynomial(zz)).1^2
    + Parent(MinimalPolynomial(zz)).1 - 5;
point_x :=
    -194925411292797456462453/638959327494406963489*zz
    - 1630328988505259813900882/1916877982483220890467;
point_y :=
    17652564032100059143825/1916877982483220890467*zz
    + 147955966498089193522369/5750633947449662671401;
assert Evaluate(equations_for_BaseP1, [point_x, point_y]) eq 0;
PP := BaseP1![point_x, point_y, K!1];
...
E := EllipticCurve(C);
assert jInvariant(E) eq K!(-250888806400/56807829);
