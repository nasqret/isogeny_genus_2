"""
Recover and certify both degree-13 elliptic maps on the Frey-Kani quotient.

The quotient curve is first reconstructed from a graph anti-isometry on
E1[13] x E2[13].  The dual kernel is converted from Kummer theta
coordinates to Mumford divisors.  Exact Jacobian sums over F_(8009^24)
remove the sign ambiguity in level-2 normal addition.  Thirty-four
deterministic point evaluations then recover the two elliptic X-coordinates
by rational interpolation.  The Y-coordinates are forced by the elliptic
equations, and the pullbacks of invariant differentials are checked to be
linear.
"""
...
elliptic_curves = [
    EllipticCurve(finite_field, [5553, 5419]),
    EllipticCurve(finite_field, [2531, 1402]),
]

hasse_power = source_polynomial^((8009 - 1)//2)
hasse_witt = matrix(
    finite_field,
    [
...
assert hasse_witt*vector(finite_field, [1, 3779]) == (
    finite_field(67)*vector(finite_field, [1, 3779])
)
assert hasse_witt*vector(finite_field, [1, 7873]) == (
    finite_field(-102)*vector(finite_field, [1, 7873])
)
...
anti_isometry_matrix = matrix(GF(13), [[1, 0], [0, 3]])
...
assert all(
    ZZ(13)*divisor == rosenhain_jacobian_24(0)
    for divisor in kernel_divisors_24
)
...
output = root / "results" / "sage_degree13_maps.json"
output.write_text(json.dumps(result, indent=2) + "\n")

print("DEGREE13_MAPS verified")
