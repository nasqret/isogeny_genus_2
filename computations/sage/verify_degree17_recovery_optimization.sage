"""Verify exact degree-17 map equivalence and the B022 timing gain."""
...
root = Path.cwd()
baseline = json.loads(
    (root / "results" / "sage_degree17_maps.json").read_text()
)
optimized = json.loads(
    (root / "results" / "sage_degree17_extension_roots.json").read_text()
)
...
result = {
    ...
    "optimized": {
        ...
        "dual_isogeny_seconds": optimized["phase_timings_seconds"][
...
output = root / "results" / "sage_degree17_recovery_optimization.json"
output.write_text(json.dumps(result, indent=2) + "\n")
print("DEGREE17_RECOVERY_OPTIMIZATION verified")
print(
    "runtime_reduction_percent",
